...
| 서비스 배포 수정 | `projects/` | 현재 `notification-backend/dev`만 존재 |
...
- 현재 서비스는 `notification-backend/dev` 하나다.
...
- Cloudflare는 `sqr.kr`, `*.sqr.kr`을 Envoy LoadBalancer 공인 IP로 proxied 처리한다.
